...
export default function TechStack() {
  return (
    <section id="stack" className="relative py-24 sm:py-32 border-t border-white/5">
      ...
      <div className="pointer-events-none absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[400px] rounded-full bg-gradient-to-r from-green-500/[0.06] to-cyan-500/[0.06] blur-[150px]" />
          ...
          >
            {[...marqueeItems, ...marqueeItems].map((item, i) => {
              ...
              return (
                <div
                  key={i}
                  className="shrink-0 flex items-center gap-2.5 rounded-full border border-white/10 bg-white/[0.03] px-4 py-2 text-xs text-zinc-300 font-mono shadow-sm hover:border-white/20 transition-colors"
                >
            ...
            return (
              ...
              >
                <SpotlightCard
                  ...
                  className="p-5 h-full flex items-center gap-4 hover:border-white/25 transition-all shadow-sm"
                >
                  <div className="flex items-center justify-center w-12 h-12 rounded-xl bg-white/[0.04] border border-white/10 shrink-0 shadow-inner px-2">
                    <LogoComponent className="w-6 h-6 object-contain" />
                  </div>
                  <div className="min-w-0">
                    <div className="font-semibold text-sm text-white tracking-tight">{s.name}</div>
                    <div className="text-xs text-zinc-400 font-mono mt-0.5">{s.role} · <span className="text-zinc-500">{s.note}</span></div>
                  </div>
                </SpotlightCard>
              </motion.div>
            );
