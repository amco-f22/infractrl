...
export function ProvisioningCell() {
  return (
    <SpotlightCard
      ...
      className="lg:col-span-2 lg:row-span-2 p-6 flex flex-col"
    >
      ...
      <div className="mt-5 flex-1 rounded-xl border border-white/10 bg-[#0a0a0c] min-h-[220px] shadow-inner overflow-hidden">
        <TerminalMockup />
      </div>
    </SpotlightCard>
  );
    ...
    >
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {["PostgreSQL", "Redis", "S3"].map((r, i) => (
          <span
            key={r}
            className={`rounded-lg border px-3.5 py-1.5 text-xs font-mono transition-colors ${
      ...
      <div className="mt-3.5 flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.03] p-3">
        <span className="text-xs text-zinc-500 font-mono">Size</span>
        <div className="flex flex-1 gap-2">
          {["Small", "Medium", "Large"].map((s, idx) => (
            <div
              key={s}
              className={`flex-1 rounded-lg px-2 py-1.5 text-center text-[11px] font-mono transition-colors ${
...
export function SyncCell() {
  return (
    ...
    >
      ...
      <div className="mt-3.5 rounded-xl border border-white/10 bg-[#0a0a0c] p-3.5 font-mono text-[11px] text-zinc-300 overflow-x-auto shadow-inner">
...
export function CleanupCell() {
  return (
    ...
    >
      ...
      <div className="mt-3.5 flex items-center justify-between rounded-xl border border-white/10 bg-white/[0.03] p-3.5">
        <div className="flex items-center gap-3">
          <div className="font-mono text-2xl font-semibold tabular-nums text-white">6d 23h</div>
          <span className="text-[11px] text-zinc-500 leading-tight">until auto-destroy<br />Slack warns 24h prior</span>
        </div>
        <span className="rounded-md border border-teal-400/30 bg-teal-400/10 px-2.5 py-1 text-[11px] text-teal-200 font-mono font-medium">scheduled</span>
      </div>
    </SpotlightCard>
  );
}
